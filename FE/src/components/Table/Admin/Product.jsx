...
const ProductTable = ({
  ...
  const fetchProducts = async (page = 0) => {
    try {
      ...
      const response = await filterProducts(filter, page, pageSize);
  ...
  const handleModifyProduct = (product) => {
    ...
    const formData = {
      ...
      brand: product.brand || '',
      status: product.productStatus?.toLowerCase() || 'active',
  ...
  const handleSubmitOption = (optionData) => {
    // Xử lý lưu option cho sản phẩm optionProduct
    console.log('Option cho sản phẩm:', optionProduct, optionData);
    setShowOptionForm(false);
    setOptionProduct(null);
  };
  ...
  return (
    <div className="w-full relative">
      {/* Nếu đang show form thì chỉ hiển thị form, ẩn bảng */}
      {showForm ? (
          ...
          <div className="rounded-lg overflow-hidden border border-slate-300 bg-white font-['Roboto'] shadow-sm">
            <div className="divide-y divide-slate-300">
              ...
              <div className="grid grid-cols-10 bg-neutral-50">
                <div className="p-4 text-slate-800/90 text-sm font-semibold">ID</div>
                <div className="p-6 text-slate-800/90 text-sm font-semibold">Name</div>
                <div className="p-6 text-slate-800/90 text-sm font-semibold">Category</div>
                <div className="p-6 text-slate-800/90 text-sm font-semibold">Price</div>
                <div className="p-6 text-slate-800/90 text-sm font-semibold">Stock</div>
                <div className="p-6 text-slate-800/90 text-sm font-semibold">Status</div>
                <div className="p-6 text-slate-800/90 text-sm font-semibold">Brand</div>
                <div className="p-6 text-slate-800/90 text-sm font-semibold">Year</div>
                <div className="p-6 text-slate-800/90 text-sm font-semibold">Actions</div>
              </div>
              {/* Rows */}
              {products.map((product, idx) => (
                <div key={product.id || product.productId || idx} className="grid grid-cols-10 bg-white hover:bg-blue-50/50 transition-colors">
                  <div className="p-4 text-zinc-800 text-xs font-normal border-r border-slate-300">
                    {product.id || product.productId || `P${String(idx + 1).padStart(3, '0')}`}
                  </div>
                  <div className="p-6 text-zinc-800 text-sm font-medium border-r border-slate-300">
                    {product.name}
                  </div>
                  <div className="p-6 border-r border-slate-300">
                    <div className="flex gap-2 flex-wrap">
                      {product.category?.name ? (
                        <span className="px-3 py-1 bg-emerald-100 rounded-md text-neutral-800 text-xs font-medium">
                          {product.category.name}
                        </span>
                      ) : (
                        <span className="px-3 py-1 bg-gray-100 rounded-md text-neutral-800 text-xs font-medium">
                          No Category
                        </span>
                      )}
                    </div>
                  </div>
                  <div className="p-6 font-semibold text-blue-700 border-r border-slate-300">
                    {product.option && product.option.length > 0 ? (
                      <div className="text-xs">
                        <div className="font-medium">Từ {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(Math.min(...product.option.map(opt => opt.price)))}</div>
                        <div className="text-gray-500">({product.option.length} options)</div>
                      </div>
                    ) : (
                      'N/A'
                    )}
                  ...
                  <div className="p-6 border-r border-slate-300">
                    {product.option && product.option.length > 0 ? (
                      <div className="text-xs">
                        {(() => {
                          // Lọc các options có remaining quantity > 0
                          const validOptions = product.option.filter(opt => opt.remainingQuantity > 0);
                          const totalStock = validOptions.reduce((sum, opt) => sum + opt.remainingQuantity, 0);
                  ...
                  <div className="p-6 border-r border-slate-300">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      ...
                      product.productStatus === 'INACTIVE' ? 'bg-gray-100 text-gray-800' :
                      product.productStatus === 'DRAFT' ? 'bg-yellow-100 text-yellow-800' :
                      'bg-red-100 text-red-800'
                    }`}>
                  ...
                  <div className="p-6">
                    <div className="flex gap-2">
                      <button 
                        ...
                        className="px-3 py-1 rounded bg-blue-600 text-white hover:bg-blue-700 text-xs font-medium transition-colors"
                      >
                        Modify
                      </button>
                      <button
                        onClick={() => handleAddOption(product)}
                        className="px-3 py-1 rounded bg-purple-500 text-white hover:bg-purple-600 text-xs font-medium transition-colors"
                      >
                        Add Option
                      </button>
                      <button
                        onClick={() => handleViewAllOptions(product)}
                        className="px-3 py-1 rounded bg-green-500 text-white hover:bg-green-600 text-xs font-medium transition-colors"
                      >
                        View All Options
                      </button>
                    </div>
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
      ...
      {showSerials && (
        <div className="mt-3 pt-3 border-t border-gray-100">
          <h4 className="text-sm font-medium text-gray-700 mb-2">Danh sách Serial:</h4>
          {option.serials && option.serials.length > 0 ? (
            <div className="space-y-1 max-h-32 overflow-y-auto">
              {option.serials.map((serial, idx) => (
                <div key={serial.id || idx} className="text-xs bg-gray-50 p-2 rounded border">
                  <div className="flex justify-between">
                  ...
                  {serial.purchaseDate && (
                    <div className="text-gray-500 mt-1">
                      Mua: {new Date(serial.purchaseDate).toLocaleDateString('vi-VN')}
                    </div>
                  )}
                </div>
              ))}
