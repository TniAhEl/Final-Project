...
const ProductOptionForm = ({ onSubmit, onCancel, productId, productInfo }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess('Thêm option thành công!');
      ...
      setErrors({ 
        general: error.response?.data?.message || 'Có lỗi xảy ra khi thêm option. Vui lòng thử lại.' 
      });
  ...
  return (
    <div className="w-[500px] max-w-full rounded-[5px] border border-violet-500 p-3 bg-white">
      {/* Header */}
      <div className="mb-3">
        <h3 className="text-base font-semibold text-gray-800">Quản lý Options cho Sản phẩm</h3>
        <div className="text-xs text-gray-600 space-y-0.5">
          <p>Product ID: {productId || 'N/A'}</p>
          {productInfo?.name && (
            <p>Tên sản phẩm: <span className="font-medium">{productInfo.name}</span></p>
          )}
          {!productId && (
            <p className="text-red-500">⚠️ Không tìm thấy Product ID</p>
          )}
        </div>
      </div>
      ...
      {existingOptions.length > 0 && (
        <div className="mb-4">
          <h4 className="text-sm font-semibold text-gray-700 mb-2">Options hiện có:</h4>
          <div className="max-h-48 overflow-y-auto">
            <div className="grid grid-cols-1 gap-2">
              {existingOptions.map((option, index) => (
                <div key={option.id || index} className="bg-gray-50 border border-gray-200 rounded-md p-3 hover:bg-gray-100 transition-colors">
      ...
      <div className="border-t border-gray-200 pt-3">
        <h4 className="text-sm font-semibold text-gray-700 mb-2">Thêm Option mới:</h4>
        <form className="flex flex-col gap-3" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col gap-1">
            ...
            />
            {errors.rom && (
              <p className="text-red-500 text-xs">{errors.rom}</p>
            )}
          </div>
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              <span>{isLoading ? 'Đang lưu...' : 'Save Option'}</span>
            </button>
          </div>
        </form>
      </div>
