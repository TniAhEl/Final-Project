  ...
  return (
    <div className="bg-yellow-50 border border-yellow-200 rounded-xl shadow p-5 flex flex-col gap-3 max-w-md">
      ...
      <div className="flex items-center gap-2 mb-1">
        <span className="bg-yellow-200 text-yellow-900 px-3 py-1 rounded-full text-sm font-semibold">-{discount}%</span>
        <span className="text-xs text-zinc-500">Expires: {expiry}</span>
      </div>
      <div className="flex items-center gap-2">
        <span className="font-mono bg-yellow-100 px-3 py-1 rounded text-yellow-900 text-base font-semibold">{code}</span>
        <button onClick={handleCopy} className="p-2 rounded bg-yellow-200 hover:bg-yellow-300 transition text-yellow-900">
          <FaRegCopy />
        </button>
